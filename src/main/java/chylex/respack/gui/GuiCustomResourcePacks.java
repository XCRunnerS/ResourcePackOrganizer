...
public class GuiCustomResourcePacks extends GuiScreenResourcePacks {
    ...
    private boolean hasUpdated, requiresReload, loadingPacks, addPacks;
    ...
    public void initGui() {
        ...
        if (!requiresReload) {
            new Thread(() -> {
                ...
                System.out.println("finished finding packs in initGui");
                loadingPacks = false;
            }).start();
        }
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 20) {
            ...
            if (requiresReload) {
                ...
                ResourcePackOrganizerRevamp.getConfig().options.updateEnabledPacks();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTickTime) {
        ...
        String text = EnumChatFormatting.WHITE + "Loading packs...";
        if (loadingPacks)
            render.drawString(text, (width / 2) - 204 + (guiPacksAvailable.width / 2) - (render.getStringWidth(text) / 2), 25, 0, true);
    }
