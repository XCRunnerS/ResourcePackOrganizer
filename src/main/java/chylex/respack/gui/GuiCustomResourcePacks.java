...
public class GuiCustomResourcePacks extends GuiScreenResourcePacks{
	...
	private List<ResourcePackListEntryFound> createAvailablePackList(ResourcePackRepository repository){
		...
		if (!repository.getDirResourcepacks().equals(currentFolder)){
			list.add(new ResourcePackListEntryFolder(this,currentFolder.getParentFile(),true)); //creates the "back" button
		}
		...
		if (files != null){
			for(File file:files){
				if ((file.isDirectory() && !new File(file,"pack.mcmeta").isFile()) || file.isDirectory() && new File(file,"folder.folder").isFile())
				{
